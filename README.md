...
O pacote compartilhado **`in_out_layout`** (`../packages/in_out_layout`) usa `defaultTargetPlatform`:

- **Windows, Linux, macOS**: layout desktop (janela maior no Windows, home em grade, módulos com barra superior e corpo até 1200px).
- **Android, iOS**: layout mobile (lista na home, `AppBar` nos módulos, `ResponsiveWrapper` só no mobile).
...
- `in_out_layout` → `../packages/in_out_layout` (form factor + shell de páginas)
